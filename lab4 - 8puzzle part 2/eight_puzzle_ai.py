...
class Graf:
    ...
    def calculeaza_h(self, reprez_nod):
        # reprez_nod este configuratia curenta
    ...
    # O varianta de euristica e aceea de a calcula distanta
    # intre locatia in reprez_nod si locatia in reprez_scop.
    def dist_euristica(self, elem, i_elem, j_elem, reprezentare):
        #...squared euclidean distance
        ind = reprezentare.index(elem)
        i_scop = ind // Nod.NR_COLOANE      # catul
    ...
    def calculeaza_succesori(self, nod):
        # Nod este o configuratie curenta
        # Pornind de la locatia curenta, se por stabili toate mutarile posibile
        ...
        if linie_gol >= 1: # daca linia este >=1, atunci sigur putem face o mutare in sus
        ...
        if linie_gol <= 1: # daca linia este <=1, atunci sigur putem face o mutare in jos
            # ...
            poz_mutare = (linie_gol + 1) * Nod.NR_COLOANE + coloana_gol
            reprez_noua = self.interschimba(ind_gol, poz_mutare, nod.info)
            h_nod = self.calculeaza_h(reprez_noua)
            l_succesori.append((Nod(reprez_noua, h_nod), 1))
        if coloana_gol >= 1: # la stanga
            # ...
            poz_mutare = linie_gol * Nod.NR_COLOANE + coloana_gol - 1
            reprez_noua = self.interschimba(ind_gol, poz_mutare, nod.info)
            h_nod = self.calculeaza_h(reprez_noua)
            l_succesori.append((Nod(reprez_noua, h_nod), 1))
        if coloana_gol <= 1: # la dreapta
            # ...
            poz_mutare = linie_gol * Nod.NR_COLOANE + coloana_gol + 1
            reprez_noua = self.interschimba(ind_gol, poz_mutare, nod.info)
            h_nod = self.calculeaza_h(reprez_noua)
