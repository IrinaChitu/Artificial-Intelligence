...
class Graf:
    ...
    def calculeaza_h(self, nod_info):
        ...
        # ... nu cred ca mai treb scris nimic altceva in calculeaza_h
    ...
    def muta_barca(self, nod_info, nr_mis, nr_can):
        ...
        # ...
        nod_copy = nod_info[:]  # de verificat daca e echivalent cu nod_info.copy()
        if nod_info[0] == 'est':    # a plecat din est, si ajunge in vest
            nod_copy[0] = 'vest'
            nod_copy[1] -= nr_mis   # est
            nod_copy[2] -= nr_can   # est
            nod_copy[3] += nr_mis   # vest
            nod_copy[4] += nr_can   # vest
        else:                       # vest => est
            nod_copy[0] = 'est'
            nod_copy[1] += nr_mis
            nod_copy[2] += nr_can
    ...
    def calculeaza_succesori(self, nod):    # calculeaza posibile mutari
        l_succesori = []
        # luam indivizii de la indicii 1 si 2 din nod.info pt malul estic, sau 3 si 4 pt malul vestic
        # ...
        for mis in range (0, Nod.NR_LOCURI_BARCA + 1):      # beacuse exclusive
            for can in range (0, Nod.NR_LOCURI_BARCA + 1):
                # in barca, daca exista misionari, numarul lor trebuie sa fie >= cu cel al canibalilor
                if 1 <= mis + can <= Nod.NR_LOCURI_BARCA and (mis >= can or mis == 0):
                    nod_copy = self.muta_barca(nod.info, mis, can)
