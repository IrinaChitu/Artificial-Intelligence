...
def elem_identice(lista):
    ...
    # ...
    lista_set = set(lista)
    if len(lista_set) == 1 and list(lista_set)[0] != '#':
        return lista_set[0]
    return False
...
def final(matr):
    ...
    '''
    # ...
    rezultat = (elem_identice(matr[0:3]) or      # linii
               elem_identice(matr[0:3]) or
               elem_identice(matr[0:3]) or
               elem_identice(matr[0:9:3]) or    # coloane
               elem_identice(matr[1:9:3]) or
               elem_identice(matr[2:9:3]) or
               elem_identice({matr[0], matr[4], matr[8]}) or    #diagonale        matr[0:9:4]
               elem_identice({matr[2], matr[4], matr[6]}))                      # matr[2:8:2]
    if rezultat:
        return rezultat
    if '#' not in matr:
        return "remiza"
